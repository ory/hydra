...
func (s *DefaultStrategy) authenticationSession(ctx context.Context, _ http.ResponseWriter, r *http.Request) (*flow.LoginSession, error) {
	...
	sessionFromCookie, _ := flowctx.FromCookie[flow.LoginSession](ctx, r, s.r.KeyCipher(), flowctx.LoginSessionCookie)
...
func (s *DefaultStrategy) verifyAuthentication(
	...
	f, err := flowctx.FromCookie[flow.Flow](ctx, r, s.r.KeyCipher(), flowctx.FlowCookie)
	if err != nil {
		return nil, err
	}
...
func (s *DefaultStrategy) verifyConsent(ctx context.Context, w http.ResponseWriter, r *http.Request, verifier string) (*flow.AcceptOAuth2ConsentRequest, error) {
...
func (s *DefaultStrategy) HandleOAuth2AuthorizationRequest(ctx context.Context, w http.ResponseWriter, r *http.Request, req fosite.AuthorizeRequester) (*flow.AcceptOAuth2ConsentRequest, error) {
	...
	if authenticationVerifier == "" && consentVerifier == "" {
		...
		f, err := s.verifyAuthentication(ctx, w, r, req, authenticationVerifier)
		if err != nil {
			return nil, err
		}

		// ok, we need to process this request and redirect to auth endpoint
		return nil, s.requestConsent(ctx, w, r, req, f)
	}

	consentSession, err := s.verifyConsent(ctx, w, r, consentVerifier)
	if err != nil {
		return nil, err
	}
...
func (s *DefaultStrategy) flowFromCookie(r *http.Request) (*flow.Flow, error) {
	return flowctx.FromCookie[flow.Flow](r.Context(), r, s.r.KeyCipher(), flowctx.FlowCookie)
}
func (s *DefaultStrategy) loginSessionFromCookie(r *http.Request) (*flow.LoginSession, error) {
	return flowctx.FromCookie[flow.LoginSession](r.Context(), r, s.r.KeyCipher(), flowctx.LoginSessionCookie)
}
