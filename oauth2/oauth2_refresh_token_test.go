...
func TestCreateRefreshTokenSessionStress(t *testing.T) {
	...
	testTokens := []signedToken{
		signedToken{
			token: "lAtgxd-seK7pUkLGuhRDaSHR9tKE39iMbIxmHhCr3IU",
			signature: "jEMGJbMdiPhaUk0JMexH5hoUdPbbWQDGD_uh2L3AXTk",
		},
		signedToken{
			token: "234c678fed33c1d2025537ae464a1ebf7d23fc4a",
			signature: "4c7c7e8b3a77ad0c3ec846a21653c48b45dbfa31",
		},
		signedToken{
			token: "7fwweMa40nPohd0J2n6Xl3Q5OtHqsNKKfYH0ajkJEiY",
			signature: "kfqPEF6a_Aw_jyymfX8jOdwSM_2Ja5LQjWZf9XNFGB8",
		},
		signedToken{
			token: "U1-PqbyTo8AxaW67YzqG3y_ZOmJG0pS9JF3j0x2jniw",
			signature: "PzZjh_-7m29J4q1pXDMPRS5ybGFiF-sq8jxeb0KVM9Y",
		},
		signedToken{
			token: "lGocOQAOprbiYnGwvlC5I3hsFOubNsKGVjC9g9VjINc",
			signature: "BaA-OPnsRrm3bWIKxNDqWjJJ2RrX9WTWSfYKhrIH23M",
		},
	}

	for _, tkn := range testTokens {
		...
		request := &fosite.AccessRequest{
			...
			Request: fosite.Request{
				...
				Form: url.Values{
					"refresh_token": []string{fmt.Sprintf("%s.%s", tkn.token, tkn.signature)},
				},
			},
		}
		...
		for dbName, dbRegistry := range registries {
			...
			require.NoError(t, dbRegistry.OAuth2Storage().CreateRefreshTokenSession(ctx, tkn.signature, request))
			_, err := dbRegistry.OAuth2Storage().GetRefreshTokenSession(ctx, tkn.signature, nil)
			...
			for run := 0; run < testRuns; run++ {
				...
				assert.NoError(t, dbRegistry.OAuth2Storage().DeleteRefreshTokenSession(ctx, tkn.signature))
				assert.NoError(t, dbRegistry.OAuth2Storage().CreateRefreshTokenSession(ctx, tkn.signature, request))
			}
		}
	}
}
