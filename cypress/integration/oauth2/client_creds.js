...
describe('The OAuth 2.0 Authorization Code Grant', function () {
  ...
  it('should return an Access Token but not Refresh or ID Token for client_credentials flow', function () {
    ...
    cy.request(
      `http://127.0.0.1:4000/oauth2/cc?client_id=${client.client_id}&client_secret=${client.client_secret}&scope=${(client.scope)}`,
      { failOnStatusCode: false },
    ).its('body').then((body) => {
      const { result, token: { access_token, id_token, refresh_token } = {} } = body
