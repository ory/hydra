...
describe('OpenID Connect Prompt', () => {
  ...
  it('should fail prompt=none when no session exists', function () {
    ...
    cy.visit(
      `http://127.0.0.1:4000/openid/code?client_id=${client.client_id}&client_secret=${client.client_secret}&prompt=none`,
  ...
  it('should pass with prompt=none if both login and consent were remembered', function () {
    ...
    cy.request(
      `http://127.0.0.1:4000/openid/code?client_id=${client.client_id}&client_secret=${client.client_secret}&scope=openid`,
    ).its('body').then((body) => {
      const { result, token: { access_token } } = body
  ...
  it('should require login with prompt=login even when session exists', function () {
    ...
    cy.request(
      `http://127.0.0.1:4000/openid/code?client_id=${client.client_id}&client_secret=${client.client_secret}&scope=openid&prompt=login`,
    ).its('body').then((body) => {
  ...
  it('should require consent with prompt=consent even when session exists', function () {
    ...
    cy.request(
      `http://127.0.0.1:4000/openid/code?client_id=${client.client_id}&client_secret=${client.client_secret}&scope=openid&prompt=consent`,
    ).its('body').then((body) => {
