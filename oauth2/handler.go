...
func (h *Handler) FlushHandler(w http.ResponseWriter, r *http.Request, _ httprouter.Params) {
	...
	if err := h.r.OAuth2Storage().FlushInactiveIDTokens(r.Context(), fr.NotAfter); err != nil {
		h.r.Writer().WriteError(w, r, err)
		return
	}

	if err := h.r.OAuth2Storage().FlushInactiveAuthorizeCodes(r.Context(), fr.NotAfter); err != nil {
		h.r.Writer().WriteError(w, r, err)
		return
	}
