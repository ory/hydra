---
id: gitea-sign-in-oauth2-openid-connect-hydra
title: Sign in to Gitea using ORY Hydra
---

Gitea (self hosted Git service) has several OAuth2 related features. The relevant here is the possibility
to sign in to Gitea with (almost) any OAuth2 provider, in this case ORY Hydra.
...
## Prepare ORY Hydra for Gitea Integration
...
We will access Gitea via the url http://gitea.example.com. So we need to map
...
domain-name to allow unencrypted http traffic. We can achieve that by passing
`--dangerous-allow-insecure-redirect-urls` to `serve all` (see the contents of `/contrib/quickstart/quickstart-gitea.yml`):
...
services:
  ...
  hydra:
    command:
      serve all --dangerous-force-http --dangerous-allow-insecure-redirect-urls http://gitea.example.com:8000/users/auth/ORY_Hydra/callback
```

## Spin up OAuth2 Server ORY Hydra and Gitea Docker Containers

Use this command to spin up the instances. This will show the logs on the
terminal and take some time.

```shell script
$ docker-compose -f quickstart.yml \
    -f quickstart-postgres.yml \
    -f contrib/quickstart/quickstart-gitea.yml \
    up --build
```
...
## Create OAuth2 Client for Gitea

Next we need to create the OAuth2 Client Gitea will use to sign in:
...
[Gitea-instance](http://gitea.example.com:3030/), you will have to register admin user
and before that in server configuration make sure you define Gitea Base URL (with `gitea.example.com:3030` in our case) access and Gitea HTTP Listen Port (`3000` in our case) which we have defined here.
