...
func TestViperProviderValidates(t *testing.T) {
	...
	assert.Equal(t, urlx.ParseOrPanic("https://device/"), c.DeviceUrl())
	...
	// self
	assert.Equal(t, urlx.ParseOrPanic("https://public/device"), c.DeviceInternalURL())
	...
	assert.Equal(t, 2*time.Hour, c.DeviceAndUserCodeLifespan())
