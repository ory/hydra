...
// servePrivilegedCmd represents the priviledged command
var servePrivilegedCmd = &cobra.Command{
	Use:   "privileged",
	Short: "Serves privileged HTTP/2 APIs",
	Long: `This command opens one port and listens to HTTP/2 API requests. The exposed API handles privileged
...
connection to be able to synchronize.`,
	Run: func(cmd *cobra.Command, args []string) {
		...
		fmt.Println("priviledged called")
	},
}

func init() {
	serveCmd.AddCommand(servePrivilegedCmd)

	// Here you will define your flags and configuration settings.

	// Cobra supports Persistent Flags which will work for this command
	// and all subcommands, e.g.:
	// servePrivilegedCmd.PersistentFlags().String("foo", "", "A help for foo")

	// Cobra supports local flags which will only run when this command
	// is called directly, e.g.:
	// servePrivilegedCmd.Flags().BoolP("toggle", "t", false, "Help message for toggle")
