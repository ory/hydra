...
describe('The OAuth 2.0 Refresh Token Grant', () => {
  before(function () { // this must be a function otherwise this.skip() fails because the context is wrong
    if (Cypress.env('jwt_enabled') !== 'true' && !Boolean(Cypress.env('jwt_enabled'))) {
      this.skip()
    }
  })
