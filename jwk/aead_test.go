...
func TestAEAD(t *testing.T) {
	...
	t.Run("case=without-rotation", func(t *testing.T) {
		c.Set(config.ViperKeyGetSystemSecret, []string{secret(t)})
		a := NewAEAD(c)
	...
	t.Run("case=with-rotation", func(t *testing.T) {
		old := secret(t)
		c.Set(config.ViperKeyGetSystemSecret, []string{old})
		a := NewAEAD(c)
		...
		c.Set(config.ViperKeyGetSystemSecret, []string{secret(t), old})
		res, err := a.Decrypt(ct)
		require.NoError(t, err)
		assert.Equal(t, plain, res)
	...
	t.Run("case=with-rotation-wrong-secret", func(t *testing.T) {
		...
		c.Set(config.ViperKeyGetSystemSecret, []string{secret(t), secret(t)})
		_, err = a.Decrypt(ct)
		require.Error(t, err)
	})
}
