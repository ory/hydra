...
A logout request can provide a `id_token_hint` and may optionally define
`state` and `post_logout_redirect_uri`. Because of that
...
- `****`: The `post_logout_redirect` defaults to the configuration value of
  `urls.post_logout_redirect`. If a
...
1. A user-agent (browser) requests the logout endpoint
   ...
   ORY Hydra as the `id_token_hint` (`/oauth2/sessions/logout?id_token_hint=...`)
   then:
