...
func (m *RegistryBase) oAuth2Config() *compose.Config {
	return &compose.Config{
		...
		DeviceAuthorisation: compose.DeviceAuthorisationConfig{
			DeviceCodeLifespan:   m.C.DeviceCodeLifespan(),
			UserCodeLifespan:     m.C.UserCodeLifespan(),
			TokenPollingInterval: m.C.DeviceAuthTokenPollingInterval(),
			VerificationURI:      m.C.DeviceAuthVerificationURI(),
		},
