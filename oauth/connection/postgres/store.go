...
var schemata = []string{
	...
	CONSTRAINT u_oauth_link_token UNIQUE (provider, subject_remote)
)`}
...
func (s *Store) Create(c Connection) error {
	_, err := s.db.Exec(
		"INSERT INTO oauth_link (id, provider, subject_local, subject_remote) VALUES ($1, $2, $3, $4)",
		c.GetID(),
		c.GetProvider(),
		c.GetLocalSubject(),
		c.GetRemoteSubject(),
...
func (s *Store) Delete(id string) error {
	_, err := s.db.Exec("DELETE FROM oauth_link WHERE id=$1", id)
	return err
...
func (s *Store) Get(id string) (Connection, error) {
	...
	row := s.db.QueryRow("SELECT id, provider, subject_local, subject_remote FROM oauth_link WHERE id=$1 LIMIT 1", id)
...
func (s *Store) FindByRemoteSubject(provider, subject string) (Connection, error) {
	var c DefaultConnection
	row := s.db.QueryRow("SELECT id, provider, subject_local, subject_remote FROM oauth_link WHERE provider=$1 AND subject_remote=$2 LIMIT 1", provider, subject)
	if err := row.Scan(&c.ID, &c.Provider, &c.LocalSubject, &c.RemoteSubject); err == sql.ErrNoRows {
		return nil, ErrNotFound
	} else if err != nil {
		return nil, err
