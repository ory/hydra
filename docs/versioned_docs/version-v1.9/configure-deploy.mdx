...
$ docker run -it --rm --entrypoint hydra oryd/hydra:v1.9.0-alpha.3.pre.2 help serve
...
$ docker run -it --rm \
  --network hydraguide \
  oryd/hydra:v1.9.0-alpha.3.pre.2 \
  migrate sql --yes $DSN
...
$ docker run -d \
  ...
  oryd/hydra:v1.9.0-alpha.3.pre.2 serve all
...
```shell
$ docker run --rm -it --entrypoint hydra oryd/hydra:v1.9.0-alpha.3.pre.2 help
