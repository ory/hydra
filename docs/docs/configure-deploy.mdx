...
$ docker run -it --rm --entrypoint hydra oryd/hydra:v1.9.0-alpha.3 help serve
...
$ docker run -d \
  ...
  oryd/hydra:v1.9.0-alpha.3 serve all
...
In this example we did not define a value for the optional setting
...
$ docker run -d \
  ...
  oryd/hydra:v1.9.0-alpha.3 serve all --config /hydra.yaml
```
...
```shell
$ docker run --rm -it --entrypoint hydra oryd/hydra:v1.9.0-alpha.3 help
...
- `photos.read` this is an imaginary scope that is not handled by ORY Hydra but
