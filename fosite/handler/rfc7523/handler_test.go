...
func (s *AuthorizeJWTGrantRequestHandlerTestSuite) TestValidAssertionCopiesAudienceWhenOmitDisabled() {
	// arrange
	ctx := context.Background()
	s.accessRequest.GrantTypes = []string{grantTypeJWTBearer}
	keyID := "my_key"
	pubKey := s.createJWK(s.privateKey.Public(), keyID)
	cl := s.createStandardClaim()
	s.handler.Config.(*fosite.Config).GrantTypeJWTBearerOmitAssertionAudience = false
	...
	s.ElementsMatch(
		...
		"audience from assertion JWT should be copied to the access request when omit_assertion_audience is false",
	)
}

func (s *AuthorizeJWTGrantRequestHandlerTestSuite) TestValidAssertionOmitsAudienceWhenOmitEnabled() {
	// arrange
	ctx := context.Background()
	s.accessRequest.GrantTypes = []string{grantTypeJWTBearer}
	keyID := "my_key"
