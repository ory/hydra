...
func newClientStore(c *configuration) client.Manager {
	if c.BackendURL == "" {
	}
...
func createAdminIfNotExists(clientStore client.Manager, ladonStore ladon.Manager, hasher hash.Hasher) error {
	...
	secret, err := sequence.RuneSequence(32, []rune("abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ1234567890_-.,:;$%!&/()=?+*#<>"))
	if err != nil {
		return errors.Errorf("Could not generate a secure secret because %s", err.Error())
	}

	hashed, err := hasher.Hash([]byte(string(secret)))
	if err != nil {
		return errors.Errorf("Could not hash secret because %s", err.Error())
	}
